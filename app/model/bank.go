...
func Bunga(transaction Transaction) (bool, error) {
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		for i, acc := range accounts {
			...
			fmt.Println(i, acc.AccountNumber, transactions.ID)

			var recipient Account
			if err := tx.Model(&Account{}).Where(&Account{AccountNumber: acc.AccountNumber}).
				First(&recipient).
				Update("saldo", recipient.Saldo+((recipient.Saldo*transaction.Amount)/100)).Error; err != nil {
				log.Println("ERROR : " + err.Error())
				return err
			}
			transactions.TransactionType = constant.BUNGA
			transactions.Timestamp = time.Now().Unix()
			transactions.Recipient = acc.AccountNumber
